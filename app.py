...
def init_db():
    ...
    for col, coltype in [('phone','TEXT'), ('email','TEXT'), ('address','TEXT'), ('commission_rate','INTEGER DEFAULT 30')]:
        if col not in cols:
...
def worker_dashboard():
    ...
    worker = query("SELECT commission_rate FROM workers WHERE id=?", (worker_id,), one=True)
    commission = int(worker['commission_rate'] if worker and worker['commission_rate'] is not None else 30)
...
def update_worker_profile():
    ...
    worker = query("SELECT id, name, role, phone, email, address, commission_rate FROM workers WHERE id=?", (worker_id,), one=True)
    return jsonify({'success': True, 'worker': dict(worker)})
...
def dashboard():
    ...
    worker_stats = query("""
        SELECT w.id, w.name, w.commission_rate, COUNT(a.id) as jobs, SUM(a.price) as earnings
        FROM workers w
        LEFT JOIN appointments a ON w.id=a.worker_id AND date(a.created_at) BETWEEN ? AND ?
        WHERE w.is_active=1
        GROUP BY w.id, w.name, w.commission_rate
        HAVING earnings > 0 OR w.role='barber'
        ORDER BY earnings DESC NULLS LAST
    """, (from_date, to_date))
    
    today_salary_total = sum((w['earnings'] or 0) * (int(w['commission_rate']) if w['commission_rate'] is not None else 30) / 100 for w in worker_stats)
    ...
    return render_template('dashboard.html',
        ...
        today_salary_total=today_salary_total,
        today_net=today_net,
        workers=workers,
        services=services,
        local_ip=local_ip
...
def report():
    ...
    worker_summary = query("""
        SELECT w.name, w.commission_rate, COUNT(a.id) as jobs, SUM(a.price) as earnings
        FROM workers w
        LEFT JOIN appointments a ON w.id=a.worker_id AND date(a.created_at) BETWEEN ? AND ?
        WHERE w.role='barber'
        GROUP BY w.id, w.name, w.commission_rate
...
def workers_api():
    if request.method == 'POST':
        ...
        if commission_rate is None or str(commission_rate).strip() == '':
            return jsonify({'success': False, 'error': 'Commission rate is required'})
        ...
        try:
            ...
            return jsonify({'success': False, 'error': 'Commission rate must be between 0 and 100'})
        ...
        wid = execute("INSERT INTO workers (name, pin, role, phone, email, address, commission_rate) VALUES (?,?,?,?,?,?,?)", 
                      (name, pin, role, phone, email, address, commission_rate))
...
def worker_detail(wid):
    ...
    try:
        ...
        return jsonify({'success': False, 'error': 'Invalid commission rate'})
        
    execute("UPDATE workers SET name=?, pin=?, role=?, phone=?, email=?, address=?, commission_rate=? WHERE id=?",
            (data['name'], data['pin'], data['role'], data.get('phone', ''), data.get('email', ''), data.get('address', ''), commission_rate, wid))
    return jsonify({'success': True})
